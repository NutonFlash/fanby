...
exports.retweetPinned = async function retweetPinned(userLink) {
  ...
  const tweet = await webdriver.wait(
    until.elementLocated(By.css(profile.tweet)),
    15000
  );
...
exports.retweetOwn = async function retweetOwn(username, retweetNumber) {
  ...
  const mediasNumber = await getNumberFromFeedHeader(username);
  ...
  while (retweetedMedia.size < mediasNumber) {
    const mediaContainers = await webdriver.wait(
      until.elementsLocated(() => {
        return container.findElements(
          By.css(`${mediaSelectors.mediaContainer} a`)
        );
      }),
      5000
    );
    ...
    for (media of filteredMedias) {
      ...
      await webdriver.wait(
        until.elementLocated(By.css(mediaSelectors.imageFrame)),
        5000
      );
 ...
 * @returns {Boolean} Return true or false
 */
async function isMyTweet(username, tweet) {
 ...
 */
async function getNumberFromFeedHeader(username) {
